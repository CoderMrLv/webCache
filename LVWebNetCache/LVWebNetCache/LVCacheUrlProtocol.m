...
@interface LVUrlCacheConfig : NSObject
...
@implementation LVUrlCacheConfig
...
+ (instancetype)instance{
    static LVUrlCacheConfig *urlCacheConfig = nil;

    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        urlCacheConfig = [[LVUrlCacheConfig alloc] init];
    });
    return urlCacheConfig;
}
- (void)clearUrlDict{
    [LVUrlCacheConfig instance].urlDict = nil;
}
...
@implementation LVCacheUrlProtocol

+ (void)startListeningNetWorking{
    [NSURLProtocol registerClass:[LVCacheUrlProtocol class]];
}

+ (void)cancelListeningNetWorking{
    [NSURLProtocol unregisterClass:[LVCacheUrlProtocol class]];
}
+ (void)setConfig:(NSURLSessionConfiguration *)config{
    [[LVUrlCacheConfig instance] setConfig:config];
}

+ (void)setUpdateInterval:(NSInteger)updateInterval{
    [[LVUrlCacheConfig instance] setUpdateInterval:updateInterval];
}
+ (void)clearUrlDict{
    [[LVUrlCacheConfig instance] clearUrlDict];
}
...
- (void)backgroundCheckUpdate{
    ...
    [[[LVUrlCacheConfig instance] backgroundNetQueue] addOperationWithBlock:^{
        NSDate *updateDate = [[LVUrlCacheConfig instance].urlDict objectForKey:weakSelf.request.URL.absoluteString];
        if (updateDate) {
            //判读两次相同的url地址发出请求相隔的时间，如果相隔的时间小于给定的时间，不发出请求。否则发出网络请求
            NSDate *currentDate = [NSDate date];
            NSInteger interval = [currentDate timeIntervalSinceDate:updateDate];
            if (interval < [LVUrlCacheConfig instance].updateInterval) {
                return;
            }
        }
...
- (void)netRequestWithRequest:(NSURLRequest *)request{
    ...
    self.session = [NSURLSession sessionWithConfiguration:config delegate:self delegateQueue:[LVUrlCacheConfig instance].forgeroundNetQueue];
    NSURLSessionDataTask * sessionTask = [self.session dataTaskWithRequest:request];
    [[LVUrlCacheConfig instance].urlDict setValue:[NSDate date] forKey:self.request.URL.absoluteString];
    [sessionTask resume];
}
